...
/// </summary>
[Shared]
[Export(typeof(IRemoteServiceProvider)), PartNotDiscoverable]
internal class ShortCircuitingRemoteServiceProvider() : IRemoteServiceProvider
{
        ...
        where TService : class
    {
        ...
        var serviceProvider = new TestTraceSourceProvider(_testOutputHelper.AssumeNotNull());

        var service = (TService)await factory.CreateAsync(stream, serviceProvider, serviceActivationOptions: default, testServiceBroker, authorizationServiceClient: default!);
        ...
        _testOutputHelper.WriteLine($"Pretend OOP call for {typeof(TService).Name}, invocation: {Path.GetFileNameWithoutExtension(callerFilePath)}.{callerMemberName}");
